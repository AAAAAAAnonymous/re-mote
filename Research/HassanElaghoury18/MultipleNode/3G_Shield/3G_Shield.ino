// This sketch only utilizes the 3G shield (no LoRa communication at all) to send a random value to either Thingspeak or <RETRACTED>. Should avoid using String objects
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        if (usercmd == "SEND\r\n") {
            ...
            //        for(i=0;i<3 && !fona.sendCheckReply(F("AT+CHTTPSOPSE=\"<RETRACTED>\",80,1"), F("OK") ,10000);i++);
