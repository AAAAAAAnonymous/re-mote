...
- A time-series database (InfluxDB) for simple data storage and a smaller footprint. Time series databases assume that insertions are more frequent than queries, so it allows for the fast insertion of large amounts of data, such as water quality data. [Learn More](https://<RETRACTED>/re-mote/publications/-/tree/master/FadhelSekerinskiYao18Timeseries)
...
2. Setup a water sensor server with one of the options below.
    ...
    - Install water sensor server on a [cloud service](https://<RETRACTED>/re-mote/publications/tree/master/HuangMengqi19Encryption&Storage).
...
2. Setup a water sensor server with one of the options below.
    ...
    - Install turtle tracker server on a [cloud service](https://<RETRACTED>/re-mote/publications/tree/master/HuangMengqi19Encryption&Storage).
